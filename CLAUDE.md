...
**Current version: 7.0.1** (Improvements-round setup — PATCH bump per the policy below, since archiving/re-scoping the PRDs is a documentation-only change) — see "Versioning" below. Keep this line current as `/bump-version` runs (that's the whole point of it).
