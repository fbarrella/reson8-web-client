...
**Current status: Phase 1–7 (the full original build-out through PWA platform polish & launch hardening) is complete and shipped as v7.0.0** — foundation, voice, advanced voice/channel management, text chat, DMs/social, admin/moderation, and PWA platform polish (installability, update flow, "What's New" modal, zero-violations accessibility gate, Lighthouse-verified performance budget, a full real-device cross-browser/cross-device matrix, deployment docs). The client is now in a post-launch **improvements round**: a set of real-usage-driven fixes and polish items tracked in `app-planning/08-improvements-round.md` — read that first for current work. The original Phase 1–7 PRD set (including the master PRD) is archived at `app-planning/archive/v7.0.0-phase-prds/` — still the authoritative historical record of *why* v7.0.0 was built the way it was, but no longer the active scope document. **Always check `app-planning/progress.txt`'s most recent entries before starting new work** — it's the authoritative record of what's actually been built and verified, and is kept current far more reliably than any status line in this file.

**Current version: 7.7.1** (IR10 — PATCH bump per the policy below, since it's a README-only copy change with no behavior change; this closes the Improvements Round) — see "Versioning" below. Keep this line current as `/bump-version` runs (that's the whole point of it).
