...
**Current version: 7.8.0** (Vercel SPA-routing fix + correct app icon — MINOR bump per the policy below, since both are fixes with no new capability) — see "Versioning" below. Keep this line current as `/bump-version` runs (that's the whole point of it).
