...
**Current version: 7.3.1** (IR4 — PATCH bump per the policy below, since it's a purely cosmetic icon swap with no behavior change) — see "Versioning" below. Keep this line current as `/bump-version` runs (that's the whole point of it).
