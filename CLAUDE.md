...
**Current version: 7.6.0** (IR8 — MINOR bump per the policy below, since it's a fix with no new capability) — see "Versioning" below. Keep this line current as `/bump-version` runs (that's the whole point of it).
